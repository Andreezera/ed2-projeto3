...
{	
	...
	if ((pout = fopen("streaming.bin","rb")) == NULL)
	{
		...
		{
			printf("N�o foi possivel abrir o arquivo principal, verifique se j� est� aberto em outro programa!");
			return 0;
		}
	}
   		...
   		switch(op){
	   		...
	   		case 2:
					if (btopen())
					{
						...
						printf("N�o foi poss�vel abrir o arquivo de ind�ces");
					}
					btclose();
				break;	   			
			...
			case 3:
				...
				{				
					...
					if(foiAchado < 1){
						printf("Chave %d%d n�o encontrada.\n", codC, codF);
					}			
				}else{
					printf("N�o foi poss�vel abrir o arquivo de ind�ces\n");
				}
				...
				break;				
			case 4:
				printf("\nDigite quantas inser��es ser�o feitas:\n");
			...
			case 5:
					if (btopen())
					{		
    					printf("\nDigite quantas buscas ser�o feitas:\n");
			   			scanf(" %d", &tempInsercoes);
...
void insereArq (int tempInsercoes, short *promo_rrn){
	...
	fclose(pout);
   	...
   	if(i!=tempInsercoes){
   		printf("\nO arquivo tinha apenas %d inser��es dispon�veis, foram inseridos %d registros\n", i, i);
   		return;
	}	
...
void buscaPArq(int tempBuscas){
  	...
  	while(posicao!=ftell(pout) && i < tempBuscas)
   	{
        ...
        key[1] = atoi(BUSCA.CodF);
	   	...
	   	if (btopen())
			...
			printf("\nN�o foi poss�vel abrir o arquivo de ind�ces\n");
	...
	if(i!=tempBuscas){
   		printf("\nO arquivo tinha apenas %d buscas dispon�veis, foram buscados %d registros\n", i, i);
   		return;
	}
